...
Reading only its own output, not the shell's. This used to merge stderr and
came back with the firmware's logging in it -- "/bin/login -- info
clickstreamHeartbeatMetricsFramework:Skipping metrics recording" -- which
is one process name and one piece of somebody else's noise, run together
...
--[[--
The startup jobs that put a login prompt on the serial line.

This is what actually holds it on these readers, and it took a screenshot to
find out. Process 1 is `init.exe` -- Amazon's upstart -- which does not read
/etc/inittab at all, so commenting out four getty lines there worked
perfectly and changed nothing. The job is `/etc/upstart/console.conf`, with
`/etc/init/console.conf` usually the same file by another name.

Returned as a list of paths, because both names may be real and both would
have to go.
--]]--
...
--[[--
Turns the console login off by moving its job aside, and back by moving it
home again.

Renamed rather than edited, so nothing has to be understood about what else
...
function Duo:setLoginPrompt(wanted)
    ...
    local device = path:gsub("^/dev/", "")
    local jobs = Duo:findConsoleJobs()
    ...
    --[[
    The jobs first, because on these readers they are the answer and the
    inittab lines are not. Process 1 is init.exe -- Amazon's upstart -- which
    never reads /etc/inittab, so an earlier version commented out four getty
    lines there, worked perfectly, and changed nothing at all.
    ]]
    local plan = {}
    ...
    if text then
        ...
        if hits > 0 then plan[#plan + 1] = T(_("  %1 line(s) of /etc/inittab"), tostring(hits)) end
    end

    if #plan == 0 then
        UIManager:show(InfoMessage:new{
            text = T(_("Nothing left to turn off: no startup job mentions a login prompt, and no uncommented line of /etc/inittab mentions one on %1.\n\nIf one is still holding the line, \"What starts the login prompt\" will say what."), device) })
        return
    end
    ...
    UIManager:show(ConfirmBox:new{
        ...
        ok_callback = function()
            ...
            if hits > 0 then
                if not slurp(backup) then spill(backup, text) end
                local ok, err = spill("/etc/inittab", table.concat(edited, "\n") .. "\n")
                if not ok then
                    UIManager:show(InfoMessage:new{
                        text = T(_("Moved %1 startup job(s), but could not write /etc/inittab: %2"),
            ...
            UIManager:scheduleIn(2, function()
                ...
                if not still then
                    ...
                    UIManager:show(InfoMessage:new{
                        text = T(_("Moved %1 job(s), but %2 is still holding %3:\n\n  %4\n\nSomething else is starting it. \"What starts the login prompt\" is the place to look."),
                            tostring(moved), still, path, Duo:nameOfProcess(still) or "?") })
                end
            end)
        end,
    })
        ...
        },
        {
            ...
            sub_item_table = {
                ...
                },
                {
                    ...
                    help_text = _("Software flow control (XON/XOFF). On, and worth leaving on.\n\nThree soldered pads carry no RTS or CTS, so without it there is no flow control at all: the far end holds four kilobytes, and an e-ink refresh stops a reader for long enough to overrun that at any speed on offer. What arrives then is a message with bytes missing from the middle of it — which reads as a pairing code that does not match, or a signature that does not check, and neither has anything to do with what actually went wrong.\n\nCalling down the wire will not show it: both devices do nothing but read for those two seconds, so nothing overruns. It only bites in use.\n\nTurn it off only for a line that is also a live console. One stray XOFF stops that port until an XON that may never come, and everything writing to the console blocks behind it. A wire whose console is still live does not work anyway, so in practice that state is behind you."),
...
On connecting, the leader's settings win. After that a change on either device moves the rest.]]),
        ...
        },
        {
                ...
                },
            {
                ...
                help_text = _("The messages that appear at the top of the screen as Duo works: connected to the other device, matched the font size, back to the book list, and the rest.\n\nOn, because the first hour with a pair is all questions — did it connect, did that setting cross — and off is where most people end up once they know the answers. Switching it off silences all of them: half of them would be a setting nobody can predict.\n\nTwo things it leaves alone. A book being copied says how far it has got either way: that is not Duo narrating itself, it is minutes of a reader doing nothing visible, and the percentage is the only evidence the wait is a transfer rather than a fault. And so do the handful of things that have gone wrong and need an answer — a refused pairing code, a book that could not be saved.\n\nThe log is written whichever way this is set, so turning the talking off does not turn the record off."),
